/* encoding: utf-8 */
/* 本文件改编自英特尔 TPM2.0-TSS 源码包提供的集成测试程序: */
/* https://github.com/01org/TPM2.0-TSS/blob/master/test/integration/asymmetric-encrypt-decrypt.int.c */
/* 编码风格沿用英特尔开发者原始风格: 1.使用4个空格取代制表符; 2.变量名全小写并用下划线'_'分隔; 3.文件名全小写, 文件名中不使用下划线, 单词之间以'-'短横线分隔 */
...
int test_invoke (TSS2_SYS_CONTEXT *sapi_context)
{
    ...
    *(UINT32 *)&(in_public.t.publicArea.objectAttributes) = 0;
    ...
    {
        print_fail("CreatePrimary FAILED! Response Code : 0x%x\n", rc);
        return ((int) rc);
    }
    ...
    *(UINT32 *)&(in_public.t.publicArea.objectAttributes) = 0;
    ...
    if (rc != TPM_RC_SUCCESS)
    {
        print_fail("Create FAILED! Response Code : 0x%x\n", rc);
        return ((int) rc);
    }
    ...
    if (rc != TPM_RC_SUCCESS)
    {
        print_fail("Load FAILED! Response Code : 0x%x\n", rc);
        return ((int) rc);
    }
    ...
    if(rc != TPM_RC_SUCCESS)
    {
        print_fail("RSA_Encrypt FAILED! Response Code : 0x%x\n", rc);
        return ((int) rc);
    }
    ...
    if(rc != TPM_RC_SUCCESS)
    {
        print_fail("RSA_Decrypt FAILED! Response Code : 0x%x\n", rc);
        return ((int) rc);
    }
